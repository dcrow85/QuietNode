...
export interface QuietNodeMCPConfig {
  apiUrl: string;
}
...
function resolveDescription(value: any): string {
  if (value?._def?.description) return value._def.description;
  if (value?._def?.innerType?._def?.description) return value._def.innerType._def.description;
  return '';
}
...
export async function createQuietNodeMCPServer(config: QuietNodeMCPConfig) {
  const allTools: Tool[] = quietnodeTools.map(convertTool);

  const server = new Server(
    { name: 'quietnode-mcp-server', version: '0.1.0' },
    { capabilities: { tools: {} } },
  );
...
async function main() {
  const config: QuietNodeMCPConfig = {
